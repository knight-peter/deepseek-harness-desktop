...
```sh
pnpm install
DSH_CHECKOUT=/path/to/deepseek-harness pnpm dev
```

- `DSH_CHECKOUT`：指向 deepseek-harness 源码目录（开发模式，使用其已构建的 `apps/cli/lib/bin.js`）。
- `DSH_ENGINE_BIN`：显式指定 dsh CLI 入口，优先级最高（调试用）。
