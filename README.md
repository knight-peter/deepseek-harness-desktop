...
- **管理窗口**（`Cmd/Ctrl + ,` 或主窗口「管理」按钮）：三个页签——**插件管理**（npm / 本地目录 / git 安装、版本列「最新 / 可更新」与一键更新、卸载）、**设置与更新**（应用更新检查/更新源、引擎版本检查/备份/重装，均有进度与结果反馈）、**开发调试**（开发功能合集：新建插件脚手架、开发模式引擎来源、引擎调试端口、引擎日志、配置树与 diff、补丁编辑器，页签顶部 `?` 有总帮助）。
