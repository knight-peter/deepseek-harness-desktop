...
- **应用更新** = electron-updater 换壳本身，**双更新源**：
  ...
  - **GitCode 国内镜像**（`https://gitcode.com/{owner}/{repo}/releases/download/latest/` 固定 `latest` release，由 `scripts/sync-domestic.mjs` 维护）。背景：GitHub 国内访问不稳定，镜像让国内用户直连 GitCode CDN（P1 实测附件下载 5.8MB/s；GitHub 大文件国内实测拉不动）。同步时机：**发版时 CI 自动同步**（build.yml 的 `sync` job，在 GitHub 内网拉自身 release 资产 → 上传 GitCode，覆盖 arm64/win/linux）；**本机 publish-x64 后手动触发 `sync-domestic` workflow 补 x64**（x64 是本机合并的，CI 时还不存在）。详细设计与 P1 实测记录见 `docs/plans/国内发布与多更新源方案.md`。
...
| `pnpm run sync-domestic` | 同步产物到 GitCode latest release（`--from-github` 从 GitHub 拉全平台；默认本机架构；GITCODE_TOKEN 从 `.env.local` 读） | 发版镜像 GitCode（§4.7）。日常走 CI workflow 自动同步，本机跑用于应急/调试 |
...
| 出正式发布物 | `pnpm run release`（自动累加版本 + commit + tag + push，可加 `--minor`/`--major`/`--version x.y.z`）→ CI 三平台打包并自动建 release（tag 走 `--publish onTagOrDraft`）并**自动同步 GitCode 镜像**（build.yml 的 sync job）→ **Intel Mac 包补发**：本机 `pnpm build` 后 `pnpm run publish-x64 --tag vX.Y.Z` 合并进同一 release（§4.6.2，token 在 `.env.local`）→ 手动触发 `sync-domestic` workflow 把 x64 补进 GitCode 镜像 → 应用内 electron-updater 按机器架构从可用更新源（GitCode 优先 / GitHub 兜底，§4.7）自动拉取对应包 |
