...
- **Intel x64 包发布**（CI 只出 arm64，因为 GitHub 已无 Intel macOS runner）：本机 `pnpm build` 出 x64 产物后，跑 `pnpm run publish-x64 --tag v0.1.0`（**不带 `--`**——pnpm 会把 `--` 原样追加进脚本 argv；GH_TOKEN 从 `.env.local` 自动读，无需 export）上传 x64 dmg/zip 到 CI 建的 release，并自动合并进 `latest-mac.yml`（electron-updater 按文件名里的 `x64`/`arm64` 匹配架构，用户端自动选对包）。**必须在 CI 创建 release 后执行**（脚本第一步查 `GET /releases/tags/<tag>`，release 不存在则 404 失败）。脚本纯 Node 无 gh 依赖，用 GitHub REST API。
