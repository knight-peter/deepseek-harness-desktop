...
| `pnpm build` | compile + electron-builder 出**当前平台**产物到 release/（mac 产物 dmg+zip） | 打本地自测包；有签名/公证凭据时自动签名公证（§4.6.1），无凭据则跳过 |
...
| 出正式发布物 | 打 git tag（如 `v0.2.0`）推 GitHub → CI 三平台矩阵出产物并自动发布到 GitHub Releases（publish 已配，§4.7；mac 签名+公证 secrets 就位则自动执行）→ 应用内 electron-updater 即可拉取更新 |
