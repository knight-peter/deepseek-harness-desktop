...
| 出正式发布物 | `pnpm run release`（自动累加版本 + commit + tag + push，可加 `-- --minor`/`--major`/`--version x.y.z`）→ CI 三平台打包并自动建 release（tag 走 `--publish onTagOrDraft`）→ **Intel Mac 包补发**：本机 `pnpm build` 后 `pnpm run publish-x64 --tag vX.Y.Z` 合并进同一 release（§4.6.2，token 在 `.env.local`）→ 应用内 electron-updater 按机器架构自动拉取对应包 |
