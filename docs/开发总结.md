...
> 面向后续接手维护的开发者。读完本文你应能回答：这个仓库是怎么运转的、改一处代码要跑什么验证、dsh 插件怎么开发与调试。更完整的决策历史（AD-1 ~ AD-10、实施阶段、风险表）见 `docs/实施计划.md`，本文不再重复论证过程，只保留结论与「踩过的坑」。
...
| 应用更新 | electron-updater | 壳自身更新走 GitHub Releases + GitCode 国内镜像（§4.7）；引擎更新走 install-engine 重装（§4.7） |
...
│  │  ├─ updateSources.ts        # 应用更新源（GitHub + GitCode 镜像）与可达性探测（§4.7）
...
│  ├─ publish-x64.mjs            # 本机 Intel x64 产物上传 + latest-mac.yml 合并（§4.6.2）
│  ├─ sync-domestic.mjs          # 同步产物到 GitCode latest release（国内更新镜像，§4.7）
│  └─ bump-version.mjs           # 发布入口：自动累加版本 + commit + tag + push（§8）
...
- **签名策略（默认不签名，杜绝误用钥匙串证书）**：共享 `.env` 里设 `CSC_IDENTITY_AUTO_DISCOVERY=false`——electron-builder **不自动搜钥匙串**（否则会抓到钥匙串里任何 Developer ID 证书，包括**公司证书**，导致个人项目被错误签名）。`mac.identity` **不设置**（不是 null——null 会连 `CSC_LINK` 一起挡掉）。**有个人证书后本机签名**：在 `.env.local` 填 `CSC_LINK`（p12 的 base64）+ `CSC_KEY_PASSWORD`，`pnpm build` 自动签名，无需改代码。CI 通过同名 secrets 注入证书。
...
- **Intel x64 包发布**（CI 只出 arm64，因为 GitHub 已无 Intel macOS runner）：本机 `pnpm build` 出 x64 产物后，跑 `pnpm run publish-x64`（**`--tag` 可省略——自动探测最新 `v*` tag**；也可显式 `--tag v0.1.0`。GH_TOKEN 从 `.env.local` 自动读，无需 export）上传 x64 dmg/zip 到 CI 建的 release，并自动合并进 `latest-mac.yml`（electron-updater 按文件名里的 `x64`/`arm64` 匹配架构，用户端自动选对包）。**上传后自动校验 release 多平台完整性**（mac x64+arm64 dmg/zip、windows exe、linux AppImage、latest-*.yml 更新元数据；缺失打印 ⚠️ 清单）**并自动把 draft release 发布为公开**（`--keep-draft` 跳过）。**必须在 CI 创建 release 后执行**（CI 建的 release 是 draft；脚本支持 draft——`/releases/tags/<tag>` 对 draft 返回 404，需从 `/releases` 列表匹配）。上传走 **HTTP/1.1**（node:https）：Node fetch 的 HTTP/2 上传大文件到 GitHub 会 `ERR_HTTP2_STREAM_ERROR`。脚本纯 Node 无 gh 依赖，用 GitHub REST API。
...
- **应用更新** = electron-updater 换壳本身，**双更新源**：
  - **GitHub Releases**（默认源，`electron-builder.yml` 的 `publish` 配置；发布策略为 electron-builder 默认 `onTagOrDraft`——CI 带 `GH_TOKEN` 且存在 git tag 或 draft release 时才发，普通 `pnpm build` 永不发布）。mac 更新依赖 zip target（已配 dmg+zip）。**发新版本 = 打 tag 推 GitHub + CI 出产物自动挂到 Releases**，`latest-mac.yml` / `latest.yml` 由 electron-builder 生成并上传；
  - **GitCode 国内镜像**（`https://gitcode.com/{owner}/{repo}/releases/download/latest/` 固定 `latest` release，由 `scripts/sync-domestic.mjs` 维护）。背景：GitHub 国内访问不稳定，镜像让国内用户直连 GitCode CDN（P1 实测附件下载 5.8MB/s）。同步方式（**本机维护**，三个模式）：① 本地模式 `pnpm run sync-domestic`——直传本机架构产物（Intel Mac → x64，全自动）；② `--dir <path>`——上传已下载好的产物（原文件名，用于 arm64/win/linux 完整镜像；下载用 `pnpm run download-release` 或 `gh release download`）；③ `--from-github`——从 GitHub 拉全平台（**需 GitHub 可达网络**，国内直连与 GitHub CI 均实测不可行）。⚠️ 曾尝试 GitHub CI 自动同步（build.yml sync job），实测 GitHub 美国 runner → 华为云 OBS 上传 180s 零进展，已移除。镜像现状：x64-only；arm64/win/linux 用户走 GitCode 无匹配架构 → 应用内降级 GitHub。详细设计与 P1 实测记录见 `docs/plans/国内发布与多更新源方案.md`。

**自动更新体验（`setupAutoUpdater()` + `checkUpdatesWithFallback()`，index.ts）**：更新源由 `updateSources.ts` 决定（GitHub / GitCode 两条配置 + 可达性探测）。默认 `auto`：先探测 GitCode（2.5s 超时）→ 可达则用 GitCode，否则用 GitHub；检查失败自动降级 GitHub 重试一次。管理窗口「设置与更新」可固定源（GitCode/GitHub）并测速，选择持久化到 settings（`updateSource` 字段，`auto`/`gitcode`/`github`）。行为：启动检查**静默**（不弹窗打扰）；发现新版后台自动下载（`autoDownload=true`）；下载完成弹「立即重启 / 稍后」——立即重启走 `app.quit()`（`before-quit` 先停引擎再放行，`autoInstallOnAppQuit` 安装），稍后则在下次退出时自动装。手动「检查应用更新」用 `manualUpdateCheck` 标志区分，明确提示「发现新版本」或「已是最新版本」；错误只记日志不打扰。⚠️ 事件监听必须在 `boot()` 里注册一次（`setupAutoUpdater()`），且仅 `app.isPackaged` 时生效（dev 模式无更新源）。
...
| `pnpm run sync-domestic` | 同步产物到 GitCode latest release（默认本机架构；`--dir` 上传已下载产物；`--from-github` 拉 GitHub 全平台，需 GitHub 可达网络；GITCODE_TOKEN 从 `.env.local` 读） | 发版后镜像 GitCode（§4.7），本机维护 |
| `pnpm run download-release` | 用 gh CLI 下载 GitHub release 产物到 release/mirror（dmg/zip/exe/AppImage/latest-*.yml，过滤 blockmap；需 `gh` 已装且登录） | 完整镜像的前置下载步骤（§4.7） |
...
| 出正式发布物 | `pnpm run release`（自动累加版本 + commit + tag + push，可加 `--minor`/`--major`/`--version x.y.z`）→ CI 三平台打包并自动建 release（tag 走 `--publish onTagOrDraft`）→ **Intel Mac 包补发**：本机 `pnpm build` 后 `pnpm run publish-x64 --tag vX.Y.Z` 合并进同一 release（§4.6.2，token 在 `.env.local`）→ **GitCode 镜像**：本机 `pnpm run sync-domestic`（x64 直传；完整镜像用 `--dir` 上传手动下载的 arm64/win/linux 产物）→ 应用内 electron-updater 按机器架构从可用更新源（GitCode 优先 / GitHub 兜底，§4.7）自动拉取对应包 |
